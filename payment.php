<?php
    ...
    if($count == 1){
        ...
        // $sql_booked = "INSERT INTO `booked` (`id_ticket`, `id_user`, `movie_name`, `pre_date`, `pre_time`, `discount`, `seats`, `price`, `room`) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stm = $conn->prepare($sql_booked);
        $stm->bind_param("sssssssss", $id_ticket, $_SESSION['id'], $movie_name, $date, $time, $discount, $seats, $price, $room);
...
<body>
    ...
    <div class="navbar">
        ...
        <div class="container">
            <form action="<?php echo 'payment_handle.php' . '?id_ticket=' . $id_ticket ;?>" method="post">
                <div class="row">
                    <div class="col">
                    ...
                    <div class="col">
                        ...
                        <div class="inputBox">
                            ...
                            <?php $count=0; $price=0; $a = str_replace(",", " ", $seats) ;
                                    ...
                                    $price = $count*50000;
                                    echo '-- ' . $price .' VND';                                    
                                    $sql_booked = "UPDATE `booked` SET `price` = (`price` + '$price') WHERE `id_ticket` LIKE '$id_ticket'";
                                    mysqli_query($conn,$sql_booked);
                            ?>
                            </span>
                        </div>
                ...
                <div class="alert">
                    <input type="submit" value="Thanh Toán" class="submit-btn" id="<?php echo $id_ticket; ?>" onclick="showSuccess()" name="payment">
                </div>
